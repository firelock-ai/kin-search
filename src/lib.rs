...
mod tests {
    ...
    fn rebuild_all_owned_parallel_matches_serial_byte_for_byte() {
        type Canon = (
            ...
            Vec<(String, usize, Vec<(u64, Vec<f32>)>)>,
            // forward docs: id -> (doc_length, tokens_by_field)
            Vec<(u64, usize, Vec<(String, f32)>)>,
            usize, // doc_count
        ...
        fn canonical(idx: &TextIndex<TestId>) -> Canon {
            ...
            let mut postings: Vec<(String, usize, Vec<(u64, Vec<f32>)>)> = index
                .iter()
                .map(|(term, p)| {
            ...
            let mut doc_rows: Vec<(u64, usize, Vec<(String, f32)>)> = docs
                .iter()
        ...
        for i in 0..n {
            ...
            let signature =
                format!("fn parse_table_{}(row: usize) -> Result<Html_{}>", i % 11, i % 5);
        ...
        let borrowed: Vec<(TestId, Vec<(&str, f32)>)> = owned
            ...
            .map(|(id, fields)| {
                (
                    *id,
                    fields.iter().map(|(t, w)| (t.as_str(), *w)).collect::<Vec<_>>(),
                )
            })
            .collect();
